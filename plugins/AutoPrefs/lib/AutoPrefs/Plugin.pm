...
sub _apply_prefs {
    ...
    foreach my $plugin_id ( keys %$plugins ) {
        ...
        MT->log(
            {
                blog_id => $blog->id,
                message => "The AutoPrefs plugin is configuring "
                  . $plugin->name . " for "
                  . $blog->name . ".",
                level => MT::Log::INFO(),
            }
        ...
        my $data  = $pdata->data() || {};
        my $datas =
          MT->registry('blog_preferences')->{$pid}->{'plugin_data'}
          ->{$plugin_id};
            ...
            MT->log(
                {
                    blog_id => $blog->id,
                    message =>
"The AutoPrefs plugin tried to set a blog preference $col that does not exist.",
